    ...
    <div className="w-full h-full min-h-screen flex flex-col" style={{ background: BG, color: TEXT, fontFamily: MONO }}>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex flex-col shrink-0" style={{ width: 88, borderRight: `1px solid ${BORDER}` }}>
          ...
          {song.layers.map((l, i) => (
            <div key={i} className="flex items-center justify-between px-1.5 gap-1"
              style={{ height: ROW_H, borderBottom: `1px solid ${BORDER}`, background: i % 2 ? "transparent" : PANEL }}>
              <span className="text-[9px] truncate" style={{ color: l.muted ? MUTED : TEXT }}>{l.name}</span>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => toggleMute(i)}>
                  {l.muted ? <VolumeX size={11} color={MUTED} /> : <Volume2 size={11} color={ACCENT2} />}
                </button>
                <button onClick={() => removeLayer(i)}><Trash2 size={11} color={DANGER} /></button>
              </div>
            </div>
          ))}
        </div>

        <div ref={scrollRef} onClick={gridClick} className="flex-1 overflow-auto relative" style={{ background: BG }}>
          <div style={{
            width: maxTick * COL_W, height: 22, position: "sticky", top: 0, zIndex: 2,
            background: PANEL, borderBottom: `1px solid ${BORDER}`,
          }}>
            {Array.from({ length: Math.ceil(maxTick / 4) }).map((_, i) => (
              <span key={i} className="absolute text-[8px] tabular-nums" style={{ left: i * 4 * COL_W + 2, top: 4, color: MUTED }}>
                {i * 4}
              </span>
            ))}
          </div>
          <div className="relative" style={{
            width: maxTick * COL_W, height: song.layers.length * ROW_H,
            backgroundImage: `repeating-linear-gradient(to right, ${GRID_LINE} 0, ${GRID_LINE} 1px, transparent 1px, transparent ${COL_W}px), repeating-linear-gradient(to bottom, ${GRID_LINE} 0, ${GRID_LINE} 1px, transparent 1px, transparent ${ROW_H}px)`,
          }}>
            {notes.map((n) => {
              const inst = instrumentById(n.instrument);
              const sel = n.id === selectedNoteId;
              return (
                <div key={n.id}
                  onClick={(e) => { e.stopPropagation(); setSelectedNoteId(n.id); setSelectedInstrument(n.instrument); setLastKey(n.key); }}
                  className="absolute flex items-center justify-center text-[7px] tabular-nums"
                  style={{
                    left: n.tick * COL_W + 2, top: n.layer * ROW_H + 3, width: COL_W - 4, height: ROW_H - 6,
                    background: inst.color, color: "#171310", borderRadius: 2,
                    outline: sel ? `2px solid ${TEXT}` : "none", outlineOffset: 1,
                    opacity: song.layers[n.layer] && song.layers[n.layer].muted ? 0.35 : 1,
                  }}>
                  {n.key}
                </div>
              );
            ...
            {isPlaying && (
              <div className="absolute top-0 bottom-0" style={{ left: playhead * COL_W, width: 1.5, background: DANGER, zIndex: 1 }} />
            )}
          </div>
        </div>
      </div>
      ...
      {/* Selected note info */}
      <div className="px-3 py-2 border-t flex items-center justify-between" style={{ borderColor: BORDER, background: PANEL, minHeight: 40 }}>
        {selectedNote ? (
          <>
            <div className="text-[11px] tabular-nums" style={{ color: TEXT }}>
              <span style={{ color: ACCENT }}>#{noteOrder}</span> of {notes.length} · tick <b>{selectedNote.tick}</b> · layer <b>{selectedNote.layer + 1}</b> ·{" "}
              <span style={{ color: instrumentById(selectedNote.instrument).color }}>{instrumentById(selectedNote.instrument).name}</span> ·{" "}
              key <b>{selectedNote.key}</b> ({keyToName(selectedNote.key)})
            </div>
            <button onClick={deleteSelected} className="flex items-center gap-1 px-2 py-1 text-[10px] rounded-sm shrink-0"
              style={{ background: "#2a1512", color: DANGER }}>
              <Trash2 size={11} /> hapus
            </button>
          </>
        ) : (
          <div className="text-[11px]" style={{ color: MUTED }}>
            Tap grid buat nambah note · tap note buat edit pitch/instrument · total {notes.length} notes
          </div>
        )}
      </div>

      {/* Piano */}
      <div className="overflow-x-auto border-t" style={{ borderColor: BORDER, background: PANEL }}>
        <div className="flex" style={{ width: (KEY_MAX - KEY_MIN + 1) * 22 }}>
          {Array.from({ length: KEY_MAX - KEY_MIN + 1 }).map((_, idx) => {
            const key = KEY_MIN + idx;
            const name = keyToName(key);
            const isBlack = name.includes("#");
            const inRange = key >= VANILLA_MIN && key <= VANILLA_MAX;
            const isCurrent = selectedNote ? selectedNote.key === key : lastKey === key;
            return (
              <button key={key} onClick={() => pianoTap(key)}
                className="flex flex-col items-center justify-end shrink-0 text-[7px] tabular-nums"
                style={{
                  width: 22, height: 56, borderRight: `1px solid ${BORDER}`,
                  background: isCurrent ? ACCENT : isBlack ? "#0d0b09" : SURFACE,
                  borderBottom: inRange ? `2px solid ${DANGER}` : "2px solid transparent",
                  color: isCurrent ? "#171310" : isBlack ? MUTED : TEXT,
                  paddingBottom: 3,
                }}>
                <span>{name.replace("#", "♯")}</span>
                <span style={{ color: isCurrent ? "#171310" : MUTED }}>{key}</span>
              </button>
            );
          })}
        </div>
...
// ── theme tokens (dark neutral, monospace — per house style) ───────────────
const BG = "#121212", PANEL = "#181818", SURFACE = "#1e1e1e", BORDER = "#2a2a2a";
const TEXT = "#e8e8e4", MUTED = "#7a7a76", GRID_LINE = "#1d1d1d";
const ACCENT = "#c98a3e", ACCENT2 = "#6fae94", DANGER = "#a8503f";
const MONO = "'IBM Plex Mono', ui-monospace, monospace";
const FONT_IMPORT = `@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@400;500;600&display=swap');`;
